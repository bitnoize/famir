import { RepositoryContainer, Session, SessionRepository } from '@famir/domain'
...
export class RedisSessionRepository extends RedisBaseRepository implements SessionRepository {
  constructor(validator: Validator, logger: Logger, connection: RedisDatabaseConnection) {
    super(validator, logger, connection, 'session')
  }
  ...
  async create(
    ...
    try {
      const [status, rawSession] = await Promise.all([
        this.connection.session.create_session(campaignId, id, secret),

        this.connection.session.read_session(campaignId, id)
      ])
  ...
  async read(campaignId: string, id: string): Promise<Session | null> {
    try {
      const rawSession = await this.connection.session.read_session(campaignId, id)
  ...
  async auth(campaignId: string, id: string): Promise<RepositoryContainer<Session>> {
    try {
      const [status, rawSession] = await Promise.all([
        await this.connection.session.auth_session(campaignId, id),
  ...
  async upgrade(
    ...
    try {
      const [status, rawSession] = await Promise.all([
        this.connection.session.upgrade_session(campaignId, lureId, id, secret),
